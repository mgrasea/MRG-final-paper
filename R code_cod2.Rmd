...
```
```{r for neutral}
neutral_dataset <- x[loc = neutral_snps$snp_pos]  # Subset dataset using correct SNP names
...
plot(components$Comp.1, components$Comp.2, xlab = "PC 1 - 4.11%",
     ylab= "PC 2 - 3.8%")
...
```{r}
all_pca <- prcomp(Fst.all_mod, center = TRUE, scale. = TRUE) 
```
...
```{r}
components <- as.data.frame(all_pca$x[,1:12])

plot(components$PC1, components$PC2, xlab = "PC 1 - 79.08%",
     ylab= "PC 2 - 14.32%")

text(components$PC1, components$PC2, row.names(components),
     cex=0.6, pos=4)
